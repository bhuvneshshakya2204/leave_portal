<?php
	...
	$description= "";
		...
		if($query->rowCount() > 0 && ($status== 1 || $status== 2)){
				...
				$is_cancelled= $result_both['Status'];
                $modify_posting_date= new DateTime($result_both['PostingDate']);
				$modify_posting_date= $modify_posting_date->format("d-m-Y H:i:s");
                if($result_both['Status'] == 4){     
					$modify_l_status = $result_both['Status'];
                    $modify_l_type = $result_both['LeaveType'];
                    $modify_l_from = $result_both['FromDate'];
                    $modify_l_to = $result_both['ToDate'];
                }
				...
				if($result_check['status']== "edit and leave approved" && $status== 1){
					$msg= "Edit request and leave both are already approved";
				}else if($result_check['status']== "edit disapproved" && $status== 2){
					$msg= "Edit request is already disapproved";
				}else if($is_cancelled== 3){
					$error= "This leave is cancelled";
				}else{
					//code to update history and main table
					if($status== 1){
						$status_updated= "edit and leave approved";
						$status_inword= "Approved";
						$email_content= "Your modification request and";
					}else{
						$status_updated= "edit disapproved";
						$status_inword= "Disapproved";
						$email_content= "Your modification request for";
					}
					...
					$created_at= date("d-m-Y H:i:s");
						...
						if($lastInsertId){
							if($status== 1){
								$sql= "update tblleaves set LeaveType=:l_type, FromDate=:l_from, ToDate=:l_to, Status=1, leave_order=4 where empid=:emp_id and ManagerID=:manager_id and id=:lid";
								$query = $dbh->prepare($sql);
								$query->bindParam(':l_type',$l_type,PDO::PARAM_STR);
								$query->bindParam(':l_from',$l_from,PDO::PARAM_STR);
								$query->bindParam(':l_to',$l_to,PDO::PARAM_STR);
								...
								$sql= "update tblleaves set LeaveType=:l_type, FromDate=:l_from, ToDate=:l_to, Status=2, leave_order=5 where empid=:emp_id and ManagerID=:manager_id and id=:lid";
								$query = $dbh->prepare($sql);
								$query->bindParam(':l_type',$l_type,PDO::PARAM_STR);
								$query->bindParam(':l_from',$l_from,PDO::PARAM_STR);
								$query->bindParam(':l_to',$l_to,PDO::PARAM_STR);
                            ...
                            $email_body= "<table><tr><td colspan='3'><strong>Dear $emp_name,</strong><br><br>\r\n";
                            $email_body.= "<strong>Your application regarding modification in leave has been $status_inword.</strong><br><br>\r\n";
                            $email_body.= "<strong>Earlier leave:</strong></td></tr>\r\n";
                            $email_body.= "<tr><td align='left'>Leave Type:</td><td>$modify_l_type</td><td>&nbsp</td></tr>\r\n";
                            $email_body.= "<tr><td align='left'>From:</td><td>$modify_l_from</td><td>&nbsp</td></tr>\r\n";
                            $email_body.= "<tr><td align='left'>To:</td><td>$modify_l_to</td><td>&nbsp</td></tr>\r\n";
                            ...
                            $email_body.= "<tr><td align='left'><br><strong>Total Leave Taken in $this_long_month $this_year_print: </strong></td><td align='left'><br>$total_leave</td><td>&nbsp</td></tr>\r\n";
				            $email_body.= "<tr><td align='left'><strong>Total LA/LE Taken in $this_long_month $this_year_print:  </strong></td><td align='left'>$total_short_leave/4</td><td>&nbsp</td></tr>\r\n";                            
                            $email_body.= "<tr><td align='left'><strong>Posting Date: </strong></td><td align='left'>$modify_posting_date</td><td>&nbsp</td></tr>\r\n";
                            
                            $email_body.= "<tr><td colspan='2'><br><font face='verdana' size='-1'><strong>Thanks,</strong><br>$manager_name</font></td><td>&nbsp</td></tr></table>\r\n";
							...
							$msg= "Edit request updated successfully";
						}else{
		...
		if($msg == "Edit request updated successfully"){
			if($_POST['pending']== 1)
