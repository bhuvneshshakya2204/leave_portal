...
.loading_style{
	...
	<div id="overlay" style="display: none;">
		<div class="loading_style">
			<?php include_once("admin/loader.php"); ?>
		</div>
	</div>
                        ...
                        <div class="card">
                            <div class="card-content">
								...
								<div style="position: absolute; top: 80px; left: 30%; width: 50%; border: 1px solid gray; z-index: 99999; background: #ffffff;">
								  ...
								  <script>
									...
									<div class="input-field col s12">
										<label for="l_type" class="label_up">Leave Type</label>
										<input id="l_type" name="l_type" type="text" style="color: black;" readonly="readonly" />
									</div>
									<div class="input-field col s12">
										<label for="l_from" class="label_up">From</label>
										<input type="text" id="l_from" name="l_from" class="datepicker" size="30" readonly="readonly" style="color: black;" />
									</div>
									<div class="input-field col s12">
										<label for="l_to" class="label_up">To</label>
										<input type="text" id="l_to" name="l_to" class="datepicker" size="30" readonly="readonly" style="color: black;" />
									</div>
									<div class="input-field col s12">
								...
								<div class="col m6">
									<div class="row">
										<div class="input-field col m6 s12">
											<span id="advance_filter"><u>Show advance filter</u></span>
										</div>
										<div class="input-field col m6 s12">
												&nbsp;		
										</div>
								...
								<form name="filter_form" id="filter_form" style="display: none;">
								<div class="col m12">
									<div class="row">
...
<?php 
$eid=$_SESSION['eid'];
$sql = "SELECT id,LeaveType,ToDate,FromDate,Description,PostingDate,AdminRemarkDate,AdminRemark,Status from tblleaves where empid=:eid order by PostingDate desc";
$query = $dbh -> prepare($sql);
...
if($stats==1){
 ...
 <?php } ?>
										...
										//$todate= "11/07/2018";
											...
											//echo $today_date= date('Y-m-d', strtotime($today_date. ' + 1 days'));
											if($diff->format("%R%a") <= 0){ ?>
												<?php if($stats==0 || $stats==1){ ?>
													<i class="material-icons icon_hover clear_icon clear_action" data-id="<?php echo $result->id; ?>">clear</i>
												<?php } ?>
												<?php if($stats== 0){ ?>
													<i class="material-icons icon_hover edit_icon edit_action" data-id="<?php echo $result->id; ?>">edit</i>
		...
		$(document).ready( function () {
			...
			$("#save_btn").click(function(){
				//code for date validation
					...
					if(from_date.length < 10 || y_from < y_current || y_from > y_current+1 || m_from < m_current || (m_from == m_current && d_from < d_current)){
					...
					var d_check = new Date(m_from +"/" + d_from +"/" + y_from);
					if(to_date.length < 10 || y_to < y_current || y_to > y_current+1 || m_to < m_current || (m_to == m_current && d_to < d_current) || new Date(m_to +"/" + d_to +"/" + y_to) < d_check){
