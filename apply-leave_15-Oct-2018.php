...
header('location:index.php');
	...
	$sql= "SELECT LeaveType, COUNT(LeaveType) AS LeaveCount FROM `tblleaves` WHERE YEAR(str_to_date(ToDate, '%d-%m-%Y')) = '".$this_year_print."' and LeaveType = 'Leave' and Status= 1 and str_to_date(ToDate, '%d-%m-%Y') < '$current_date' and empid= '".$eid."' GROUP BY(LeaveType)
	UNION
	SELECT LeaveType, COUNT(LeaveType) AS LeaveCount FROM `tblleaves` WHERE MONTH(str_to_date(ToDate, '%d-%m-%Y')) = '".$this_month."' and (LeaveType = 'Late Arrival (LA)' or LeaveType = 'Leaving Early (LE)') and Status= 1 and str_to_date(ToDate, '%d-%m-%Y') < '$current_date' and empid= '".$eid."' GROUP BY(LeaveType)";
	$query = $dbh -> prepare($sql);
	$query->execute();
...
<div class="input-field col s12">
	...
	<input type="text" name="employee_name_text" id="employee_name_text" value="<?php echo htmlentities($result->FirstName." ".$result->LastName);?>" readonly="readonly" style="color: #7F9FAF; font-family: Roboto, sans-serif; font-weight: 500; font-size: 14px; border-bottom: 1px solid #BCBCBC;" />
</div>
 <div class="input-field col  s12">
...
<!--<label for="fromdate" id="from_date" class="label_up">From  Date</label>-->
...
<!--<label for="todate" id="to_date" class="label_up">To Date</label>-->
